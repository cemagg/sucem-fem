...
meas_max = N.array([6,3,3.])*h/2*(1-1e-10)
meas_min = -meas_max
...
B_order = (order, True)
freeE = freeB = cb
system = BrickCoupledFirstOrderSystemBDirechlet(
    mesh, disc_orders=dict(E=E_order, B=B_order),
    BCs=Struct(E=freeE, B=freeB), btype='cohen98')
...
system.useLU = True
system.discs.E.diagonalise()
system.discs.B.diagonalise()
...
pickle.dump(Struct(loggedReconstructed=system.loggedReconstructed),
            file('+freespace_dipole-refltest_3rd.pickle', 'w'))
