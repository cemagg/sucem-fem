...
output_filename_template = output_dir + os.sep + 'time_icc_redhybmesh_%s_o%d_dt%f.pickle'
...
#                               if 2**i > dt_div_min]
    ...
    TRS.init_systems(reduced_tet_order=1)
    ...
    for dt_div in dt_divs:
        ...
        #             continue
        TRS.set_dt(dt)
        TRS.hybrid_system.useLU = False
        from NewCode.IOUtils import mm_write
        fname = '/tmp/mat%d' % random.randint(0, sys.maxint)
        A_imp = TRS.hybrid_system.merged_matrices.A_imp()
        print 'Writing A_imp'
        mm_write(file(fname, 'w'), A_imp)
        import numarray
        import getfem
        class getfem_solver(object):
            def __init__(self, getfem_mat, tol=1e-8):
                self.tol = tol
                self.mat = getfem_mat
                self.n = self.mat.size()[0]
                self.b_tmp = numarray.zeros(self.n, type=numarray.Float64)
                self.x_tmp = N.zeros(self.n, dtype=N.float64)
            def init_PC(self):
                print "Calulating preconditioner"
                self.PC = getfem.Precond('ildlt', self.mat)
            def __call__(self, b):
                b_na = self.b_tmp
                b_na[:] = b
                x_np = self.x_tmp
                #print 'GETFEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEMMMM'
                x_np[:] = getfem.linsolve_cg(self.mat, b_na, self.PC, #'noisy',  
                                            'res', self.tol)
                return x_np
            
        print "Reading A_imp into getfem"
        gs = getfem_solver(getfem.Spmat('load', 'mm', fname))
        gs.init_PC()
        A_imp.solve = gs
