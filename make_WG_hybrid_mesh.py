...
joining_nodes = N.array([N.average(f.nodeCoords, axis=0) + [0,0,h/2]
...
hyb_nodes = N.vstack((hex_mesh.nodes[:], joining_nodes))
...
joining_edgenodes = make_joining_edgenodes(hex_mesh.faces[bdry_faces],
                                           joiningnode_offset=len(hex_mesh.nodes))
hyb_edgenodes = N.vstack((hexface_edgenodes, diag_edgenodes, joining_edgenodes))
hyb_edges = ProxyList.ProxyList(FakeEdge(dict(nodes=hyb_edgenodes,
                                              nodeCoords=hyb_nodes)))

hyb_tetelnodes = make_joining_elementnodes(hex_mesh.faces[bdry_faces],
                                           joiningnode_offset=len(hex_mesh.nodes))

hyb_facenodes = make_joining_facenodes(hyb_tetelnodes)
...
hyb_tetfaces = make_tetfaces(hyb_tetelnodes, hyb_facemap)

no_hexnodes = len(hex_mesh.nodes[:])
joining_nodeset = set(xrange(no_hexnodes, no_hexnodes+len(joining_nodes)))
geom_nodes = N.array([[0,0,c], [0,b,c], [a,b,c], [a,0,c]], N.float64)
...
hyb_corner_nodes = N.array([hex_mesh.findClosestNode(r)
                    for r in N.array([[0,0,c*bfrac], [0,b,c*bfrac],
                                      [a,b,c*bfrac], [a,0,c*bfrac]])],
                           N.int32)
...
geom_vol_faces = N.hstack((connecting_faces,
                           N.arange(len(geom_faceedges))+geom_face_offs))


tst_geof.writelines(nodes_to_geo(hyb_nodes,h=h*1.1))
tst_geof.writelines(edges_to_geo(hyb_edges))
tst_geof.writelines(faceedges_to_geo(hyb_faceedges))
tst_geof.writelines(tetfaces_to_geo(hyb_tetfaces))
tst_geof.writelines(nodes_to_geo(geom_nodes, offset=len(hyb_nodes), h=h*1.1))
...
tst_geof.writelines(faces_to_geo([geom_vol_faces],offset=geom_vol_offset))
tst_geof.close()
