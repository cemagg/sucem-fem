...
! And sometime between here and there, a new FEK file format was added.
...
SUBROUTINE MESHIN_FEK(infilename)
...
!*******************************************************************************
! This subroutine handles mesh input from fek files. 
...
! Changed 13 Dec 2003.  DBD. Updated input cards to current (as of Dec 2003) FEKO format,
! (MAJOR REVISIONS)          incorporating changes made by  M Bingle. Option also now available 
!                            to read old-style data files (MESHIN_FEK_OLD called instead in main program).  
!                            Also started migrating to FORMAT statements rather than call to READ_LINE,
!                            which permits fields to be interpreted as zero/null.
!
!*******************************************************************************
! INPUT 
!*******************************************************************************
...
! (new card). 
! The DI card describes the material parameters for FEA (previously MA card).
...
!!$   CHARACTER(FILENAMELENGTH) :: meshfilename, outfilename, fekoout_filename
   CHARACTER(FILENAMELENGTH), INTENT(IN) :: infilename

!!$   CHARACTER(RUN_LABEL_LINE_LENGTH), DIMENSION(RUN_LABEL_NUM_LINES), &
!!$                               INTENT(OUT) :: run_label 
   CHARACTER(8) DATE
   CHARACTER(10) TIME
   CHARACTER(15) string, string1, filestring
   CHARACTER(5) stemp1, stemp2, stemp3, stemp4, stemp5
   CHARACTER(2) card_label
...
! Added DBD 9 Dec 2003
! End added DBD 9 Dec 2003
   ...
   WRITE(FILEOUT,'(1X,A,A,3(/,1X,A))')                              &
                      'Authors:          ','D.B.Davidson, M.M.Botha, R.H.Geschke & N. Marais',   &
...
! Added DBD 12 Dec 03
   DI_counter = 0
! End added DBD 12 Dec 03
   MA_counter = 0 ! Obsolete, but retained


   !******************************************************   

   !Read header of FEK file. Changed DBD 12 Dec 2003 for new FEKO format.
   HEADER_LOOP2: DO
     READ(MESHIN,'(2X,A10)') string
! print *,string     
	 IF (string.EQ.'HEADER_END') THEN
       EXIT HEADER_LOOP2
     END IF
   END DO HEADER_LOOP2
   ...
   CARD_LOOP2: DO 
     ...
     READ(MESHIN,'(A2)',ADVANCE='NO') card_label 
     CARD_TEST: IF (card_label.EQ.'A0') THEN
       CALL A0_CARD ! Incident plane wave
     ELSE IF (card_label.EQ.'A8') THEN
       CALL A8_CARD ! CBAA current probe excitation
     ELSE IF (card_label.EQ.'A9') THEN
       CALL A9_CARD ! CBAA current probe excitation
! Added DBD 25 March 2003
     ELSE IF (card_label.EQ.'AB') THEN        
       CALL AB_CARD ! Absorbing boundary condition card
! End added DBD 25 March 2003
     ELSE IF (card_label.EQ.'AD') THEN  
       CALL AD_CARD ! ADaptive FEM control card
     ELSE IF (card_label.EQ.'AX') THEN
       CALL AX_CARD ! CBAA coax excitation (experimental)
      ELSE IF (card_label.EQ.'BC') THEN
       CALL BC_CARD ! Boundary condition
     ELSE IF (card_label.EQ.'BO') THEN  
       CALL BO_CARD ! Ground plane
! Added DBD 28 March 2003
     ELSE IF (card_label.EQ.'DP') THEN        
       CALL DP_CARD ! Define point card. 
! End added DBD 28 March 2003
! Added DBD 12 Dec 2003
     ELSE IF (card_label.EQ.'DI') THEN        
        PRINT*, "DI card now handled in auxilary input file, please remove from .fek file."
        STOP
! End added DBD 12 Dec 2003
     ELSE IF (card_label.EQ.'EA') THEN  
        CALL error_femfeko(1,5000) ! No longer support this (NM 2005.03)
        !CALL EA_CARD ! Eigenanalysis ARPACK control card.
! Added DBD 14 Dec 2003
     ELSE IF (card_label.EQ.'EG') THEN        
       CALL EG_CARD ! End of geometry card. 
! End added DBD 14 Dec 2003
     ELSE IF (card_label.EQ.'EI') THEN  
        PRINT*, "EI_card now handled in input.dat, please remove from .fek file."
        STOP
!!$       CALL EI_CARD ! Eigenanalysis 
! Added DBD 17 Dec 2003
     ELSE IF (card_label.EQ.'EO') THEN        
        PRINT*, "EO card now handled in input.dat, please remove from .fek file."
        STOP
!!$       CALL EO_CARD ! Element order card. 
! End added DBD 17 Dec 2003
     ELSE IF (card_label.EQ.'FA') THEN  
       CALL FA_CARD ! Field averaging command
     ELSE IF (card_label.EQ.'FC') THEN  
        PRINT*, "FC card now handled in input.dat, please remove from .fek file."
        STOP
!!$        CALL FC_CARD ! Finite element Control card
     ELSE IF (card_label.EQ.'FE') THEN  
       CALL FE_CARD ! Near field card. 
     ELSE IF (card_label.EQ.'FF') THEN  
       CALL FF_CARD ! FEKO Far field card. 
     ELSE IF (card_label.EQ.'FM') THEN  
!!$       CALL FM_CARD ! Finite element Matrix card
        PRINT*, "FM card now handled in input.dat, please remove from .fek file."
        STOP
     ELSE IF (card_label.EQ.'FR') THEN  
        PRINT*, "FR card now handled in input.dat, please remove from .fek file."
        STOP
!!$       CALL FR_CARD ! FEKO frequency data card
     ELSE IF (card_label.EQ.'FX') THEN  
        PRINT*, "FX card now handled in input.dat, please remove from .fek file."
        STOP
!!$        CALL FX_CARD ! Finite element eXtended control card.
     ELSE IF (card_label.EQ.'GW') THEN  
       CALL GW_CARD ! Guided wave options card
     ELSE IF (card_label.EQ.'MA') THEN  
        PRINT*, "MA card now handled in input.dat, please remove from .fek file."
        STOP
! Added DBD 25 March 2003 and 05 June 2003
     ELSE IF (card_label.EQ.'PL') THEN        
       CALL PL_CARD ! Perfectly matched layer card
     ELSE IF (card_label.EQ.'QU') THEN        
       CALL QU_CARD ! Cuboid card
     ELSE IF (card_label.EQ.'TD') THEN        
       CALL TD_CARD ! Time Domain analysis card
! End added DBD 25 March 200305 June 2003 
     ELSE IF (card_label.EQ.'TE') THEN        
       CALL TE_CARD ! Tetrahedral element card
! Changed DBD 12 Dec 2003
     ELSE IF (card_label.EQ.'EN') THEN ! End of the file card.
       EXIT CARD_LOOP2
! Changed DBD 12 Dec 2003
     ELSE ! Comment cards or presently unimplemented card.
       READ (MESHIN,'(A15)') card_label 
     END IF CARD_TEST
   END DO CARD_LOOP2
...
! Added DBD 12 Dec 03
  NUM_DI_cards = DI_counter
! End added DBD 12 Dec 03
  NUM_MA_cards = MA_counter ! Obsolete, but retained.
...
!!$  CALL material_properties_allocate(MAX_MATERIALS)
  ...
  ALLOCATE (DIdata(NUM_DI_cards+1))   ! MA card parameters
...
!*******************************************************************************
  ...
  ! NM: These are initialised in input_problem_info.f90 now
!!$  SELECT CASE (HIERARCHAL_ORDER)
!!$  CASE(1)
!!$    gauss_points = 3
!!$  CASE(2)
...
! End added DBD 28 March 2003
   ...
   CARD_LOOP1: DO 
     READ (MESHIN,'(A2)',ADVANCE='NO') string 
! print *,string	 
     IF (string.EQ.'EN') THEN ! End of FILE 
       EXIT CARD_LOOP1
     ELSE IF(string.EQ.'TE') THEN 
! Changed DBD 13 Dec 2003. TE card now only two lines. 
       30 FORMAT (3X, 5I5, 6E20.10) ! Note first field (string - TE) already read.
       READ (MESHIN,30) itemp1, itemp2, itemp3, itemp4, itemp5,&
                        xyztemp1,xyztemp2   
       31 FORMAT (A2, 3X, 5I5, 6E20.10)
       READ (MESHIN,31) card_label,itemp1, itemp2, itemp3, itemp4, itemp5,&
                        xyztemp3,xyztemp4
! End changed DBD 13 Dec 2003. 
       element_counter = element_counter+1
! Changed DBD 12 Dec 2003. Previously MA card
     ELSE IF(string.EQ.'DI') THEN
       DI_counter = DI_counter+1
! End changed DBD 12 Dec. 
...
!
! Changed DBD 17 Dec 2003 to read using format statement.
! Permits fields to  be skipped.
!***************************************************

  A0_counter = A0_counter+1
  29 FORMAT (3X, 5I5, 7E20.10) ! Note first field (string - A0) already read.
  READ (MESHIN,29) itemp1, itemp2, itemp3, itemp4, itemp5,&
...
! Time-domain (TD) and frequency-domain (FD) usage varies slightly;
! FD usage assumes a single spherical ABC surface;
! TD usage assumes an outer box ABC surface.
!
! Originally written 2003, DBD. 
! Extended DBD 21 Jul 04 to include FD-specific options.
!***************************************************
! Format I1 I2 I3 (I4) (I5) R1 (R2) R3 R4 R5
!***************************************************
! I1      : unused
! I2 =n   : Integer n is a unique label for each ABC (not used for FD solver, 
!           but should have zero "place-holders")
! I3 =1   : ABC type: 1st order. 
!    =2               2nd order (only implemented in frequency domain)
!    =-2              2nd order (ditto) but with only curl term.
!    =3   :           2nd order (ditto) but with div term (incompletely) done via surface div theorem.
! I4,I5   : Not presently used, but should have zero "place-holders"
! R1      : TD: Y_c (intrinsic impedance of infinite medium)
!           FD: Not used
! R2      : TD: unused
!           FD: radius of spherical ABC (only used by 2nd order ABC, but supply place holder for 1st order).
!               Specify a negative radius to force computation of radius in each element (eg for 
!               non-speherical surfaces). 
! R3, R4, R5
!        : TD: Outward-directed unit normal at ABC. 
!        : (x, y and z components).
!        : FD unused, but should have zero "place-holders"
!
!***************************************************
  ...
  IF(.NOT.FD_SCAT_ANALYSIS) THEN
    CALL READ_LINE(string,itemp1,itemp2,itemp3,itemp4,itemp5,rtemp1,rtemp2,rtemp3,rtemp4,rtemp5) 
  
    IF (ABS((rtemp3**2+rtemp4**2+rtemp5**2)-1.0_SP).GE.EPS) THEN ! Check for unit normal
...
!*******************************************************************************
   ...
   SUBROUTINE DI_CARD
!*******************************************************************************
! DI card : A partial implementation of the FEKO electrical characteristics card 
!*******************************************************************************
! Format I1 I2  (I3) (I4) (I5) R1 R2 R3 R4 R5 R6 
!*******************************************************************************
! I1: MED  Index of the medium as used by the ME card. Zero is surrounding free space.
! I2, I3, I4, I5: Unused, but place holders can be supplied. 
! R1: EPSR Relative permittitivity
! R2: MUER: Relative permeability
! R3: SIGMA Conductivity \sigma
! R4: TANDMUE: Magnetic loss factor \tan \delta_mu (NOT IMPLEMENTED)
! R5: TANDEPS: Electric loss factor \tan \delta = \sigma/\omega \epsilon_r \epsilon_0
!     (NOT IMPLEMENTED) 
! R6: RHO: Medium density (NOT IMPLEMENTED)
!
!*******************************************************************************


30 FORMAT (3X, 5I5, 6E20.10) ! Note first field (string - DI) already read.
   READ (MESHIN,30) itemp1, itemp2, itemp3, itemp4, itemp5,&
                    rtemp1,rtemp2,rtemp3,rtemp4,rtemp5,rtemp6
					
   IF ((ABS(rtemp3).GT.EPS).AND.(ABS(rtemp5).GT.EPS)) THEN
     STOP 'Error in DI card: cannot specify tan delta and sigma simultaneously'
   ELSE IF (ABS(rtemp6).GT.EPS) THEN
     STOP 'Unimplemented feature: RHO in DI card'
   END IF

  DI_counter = DI_counter + 1
  
  IF(itemp1.GT.MAX_MATERIALS) THEN ! Error check - this shouldn't happen.
    STOP 'IE: MAX_MATERIALS: somehow exceeded internally in subroutine DI_CARD.'
  ...
  END IF
  IF (DI_counter.EQ.1) THEN
     lowest_material = itemp1  
  ELSE IF (itemp1.LT.lowest_material) THEN
     lowest_material = itemp1
  END IF    

  DIdata(DI_counter)%medium        = itemp1
  DIdata(DI_counter)%material_type = 1 ! Only isotropic media supported.
  DIdata(DI_counter)%analysis_no   = analysis_counter + 1  ! applicable to all subsequent analysis
  DIdata(DI_counter)%eps_r_prime   = rtemp1
  DIdata(DI_counter)%mu_r_prime    = rtemp2
  DIdata(DI_counter)%sigma         = rtemp3
  DIdata(DI_counter)%tandmue       = rtemp4
  DIdata(DI_counter)%tandeps       = rtemp5
END SUBROUTINE DI_CARD
...
SUBROUTINE EG_CARD
!*******************************************************************************
! The EG card: FEKO geometry card: end of geometry section.
! This is a partial implementation. Additional parameters may be 
! entered according to specification below, but are ignored by code.  
!*******************************************************************************
! Format I1 I2 I3 0 0 R1 0 0 0 0 0 0 R8
!*******************************************************************************
! I1 = 0: The geometric data is written to the output file
!      1: No geometric data is written
!      2: Write geometric data to NASTRAN file (not available in FEMFEKO)
!      3: Write geometric data to output and NASTRAN file
!         (ONLY to output file in FEMFEKO)
! NONE OF THE FOLLOWING IS IMPLEMENTED AT PRESENT:
! I2 =-1: No messages are sent to the standard output device (screen)
!      0: Warnings, errors and messages that indicate the program's 
!         progress are sent to the standard output device
!      1: Warnings, errors and explicit messages that indicate the program's 
!         progress are sent to the standard output device. This option
!         is useful during program development and debugging and thus is
!         only available in the superuser mode.
! I3 = 1: Switch normal geometry checking off (NOT IMPLEMENTED AT PRESENT)
! R1    : Maximum identical distance (NOT IMPLEMENTED AT PRESENT)
! R8    : Flag to use single precision for FEM and preconditioner
!         matrices (NOT IMPLEMENTED AT PRESENT)
!*******************************************************************************

   CALL READ_LINE(string,itemp1) ! Only first entry 

!   CALL WRITE_CARD_OUTPUT (CODE,PFLAG,itemp1,itemp2,itemp3,itemp4,itemp5,rtemp1, &
!                           rtemp2,rtemp3,rtemp4,rtemp5,rtemp6,rtemp7,rtemp8)
   ...
   ! Output of messages to the screen?
   !SELECT CASE(itemp2)
   !CASE(-1,0,1,99) 
   !  PS5 = itemp2
   !CASE DEFAULT
   ...
   ! Extensive output only available in the superuser mode
   !IF ((PS5.EQ.1).AND.(SUMODE.NE.1)) THEN
   !  PS5 = 0
   !  CALL ERROR (0, 24090, &
   !       'Ausfuehrliche Ausgabe von Meldungen nur im Superusermodus erlaubt', &
   !       'Output of detailed messages is only allowed in the superuser mode')
   !END IF
   ! undocumented feature: PS5=99 output of detailed messages without
   ! the need of switching into SU mode
   !IF (PS5.EQ.99) PS5=1
 
   !! Print a short note if no output to the screen
   !IF (PS5.EQ.-1) THEN
   !  PS5 = 0 ! otherwise note will not be printed
   !  CALL OUTPUT_MESSAGE(' ',' ')
   !  CALL ERROR (-1, 24089, &
   !       'Keine Ausgabe von Meldungen auf den Bildschirm, siehe *.out Datei', &
   !       'No output of messages to the screen, see *.out file')
   !  CALL OUTPUT_MESSAGE(' ',' ')
   !  PS5 = -1
   !ENDIF

   !NOCHECK = itemp3
! If we are in the special mode --check-only, then do the checks in any case, 
! ignore the flag I3
   !IF (FEMFEKO_STAGES.EQ.1) NOCHECK = 0

   !IF (rtemp1.GT.0) EPSENT = rtemp1

! The parameter R8=1 indicates that we want to use single precision,
! for all other values of R8 use double precision.
!   itemp = rtemp8   ! round to integer
   !IF (itemp.EQ.1) THEN
...
!*******************************************************************************
   ...
   SUBROUTINE EN_CARD
!*******************************************************************************
! The EN card: FEKO end file card. DBD 14 Dec 03.
!*******************************************************************************
...
!***************************************************
  ...
  CASE(-8)
    IF(.NOT.((TD_ANALYSIS.AND.SCAT_FIELD).OR.(FD_SCAT_ANALYSIS))) & 
...
! Format I1 I2 I3 (I4) (I5) R1
!***************************************************
! I1 = 1: Renumbering undertaken to minimize matrix bandwidth (obsolete)
...
!*******************************************************************************
   ...
   SUBROUTINE TE_CARD
!*******************************************************************************
! TE card : FEMFEKO card for a TEtrahedral element
! Required for each tetrahedral element.
! This routine has  been upgraded to 
! read .fek files in the new (as of 2003) format. 
! However, the internal code data structures in which the nodal information is stored 
! are unchanged. 
! Also operates in SINGLE PRECISION
!*******************************************************************************
! New format of the TE_CARD: there are 2 lines:
! Format I1    I2  (I3) (I4) (I5) R1 R2 R3 R4 R5 R6 
! Format (I1) (I2) (I3) (I4) (I5) R1 R2 R3 R4 R5 R6 
!*******************************************************************************
!      1st line    card name 'TE'
!                   I1         label
!                   I2         material index (since 14-01-2003)
!                   R1/R2/R3   x/y/z  coordinates of node 1
!                   R4/R5/R6   x/y/z  coordinates of node 2
!       2nd line    R1/R2/R3   x/y/z  coordinates of node 3
!                   R4/R5/R6   x/y/z  coordinates of node 4
!*******************************************************************************
...
! print *,xyztemp1,xyztemp2

   ! Since 14-01-2003 we associate the material index with the material
   ! properties instead of the label. For backwards compatibility in the
   ! standalone version of FEMFEKO, we equal the label and material index
   ! if the material index is 0

   ! store the material index in itemp
   IF (itemp2.EQ.0) THEN 
     itemp = itemp1 ! use the label as index
   ELSE 
     itemp = itemp2
   ENDIF 
