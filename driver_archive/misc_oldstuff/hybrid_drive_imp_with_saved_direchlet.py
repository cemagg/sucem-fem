...
direch_edgeno_filter = lambda edge: not on_hybrid_boundary(edge) and cb(edge)
implicit_direch_edgenos = N.array([i for i in N.unique(
    mesh.elements[explicit_bdry_element_nos].edgenos.flat)
                          if direch_edgeno_filter(mesh.edges[i])], N.int32)
...
# All E functions in explicit region, excluding the imp/exp boundary and wg walls
test_explicit_freeE = lambda ent: cb(ent)
# All B functions in explicit, inc the imp/exp bdry and wg port, excl wg walls
test_explicit_freeB = lambda ent: (test_explicit_freeE(ent)
                             or onPort(ent))
explicit_system = CoupledFirstOrderSystemBDirechlet(
    mesh, dt=dt, BCs=Struct(E=test_explicit_freeE, B=test_explicit_freeB),
...
def get_explicit_dofnos(disc, edge_nos):
    edgeno_to_direchdofno = dict((edge_nos[i], i) for i in range(len(edge_nos)))
    direchdofno_to_explicitdofno = N.zeros(len(edge_nos), N.int32) -1
    edgeno_to_explicitdofno = {}
    p = disc.permuter.permuteElementEntities
    for el in disc.elements:
        pe = p(el, remove_constrained=False).edge[:,0]
        for edgeno, edgedofno in zip(el.edgenos, pe):
            if edgeno in edge_nos: assert(edgedofno > -1)
            else: continue
            if edgeno not in edgeno_to_explicitdofno:
                edgeno_to_explicitdofno[edgeno] = edgedofno
            else: assert (edgeno_to_explicitdofno[edgeno] == edgedofno)
    for edgeno, dofno in edgeno_to_explicitdofno.iteritems():
        direchdofno_to_explicitdofno[edgeno_to_direchdofno[edgeno]] = dofno

    return direchdofno_to_explicitdofno
            
edn = get_explicit_dofnos(explicit_system.discs.E, implicit_direch_edgenos)

#explicit_system.addLogger('E', edn)
...
test_elnos, test_el_coords = PostProc.LocatePoints(explicit_system.mesh, test_pts)
explicit_system.addReconstructedLogger('E', test_elnos, test_el_coords)
explicit_system.step(1500)

implicit_system = CurlCurlNewmarkDirechletCoupled(mesh, order=1, BC=implicit_freeE,
                                                  useQ=False)

implicit_direchSys = CurlCurlNewmark(newmark_sys.mesh, order=newmark_sys.order,
                                         BC=newmarkDirechBC, useQ=newmark_sys.hasQ)
implicit_direchSys.dof_mem = ArrayMemory(implicit_direchSys.dofs.dofArray, mem_len=3)
implicit_system.setDirechSystem(implicit_direchSys)
