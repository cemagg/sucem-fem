...
freespace_size = cube_geom_size*4 + 2*h
...
E_plane_recon_pts = N.array([[rho*N.cos(phi), 0, -rho*N.sin(phi)]
                             for phi in N.linspace(90,270,5)/180*N.pi
                             #for rho in [l, 2*l]], N.float64)
                             for rho in [2*l]], N.float64)

H_plane_recon_pts = N.array([[rho*N.cos(phi), rho*N.sin(phi), 0]
                             for phi in N.linspace(0,180,5)/180*N.pi
                             #for rho in [l, 2*l]], N.float64)
                             for rho in [2*l]], N.float64)
...
brick_mesh = BrickMesh.Mesh(
    BrickMeshGen.make_rect_cavity_brick_listmesh(x_s, y_s, z_s, h,
                                                 grid_offset=-mesh_size/2))
...
class TestRun(Runners.TestRun):
    ...
    def setupLogging(self):
        ...
        Eplane_elnos, Eplane_el_coords = LocatePoints(bondsys.mesh, E_plane_recon_pts)
        Hplane_elnos, Hplane_el_coords = LocatePoints(bondsys.mesh, H_plane_recon_pts)
        bondsys.addReconstructedLogger('E', Eplane_elnos, Eplane_el_coords )
        bondsys.addReconstructedLogger('B', Eplane_elnos, Eplane_el_coords )    
        bondsys.addReconstructedLogger('E', Hplane_elnos, Hplane_el_coords )
        bondsys.addReconstructedLogger('B', Hplane_elnos, Hplane_el_coords )    

    def getResult(self):
        bondsys = self.bonding_system
        return Struct(E_plane=Struct(
            E=N.array(bondsys.loggedReconstructed.E[0].vals)[:,0,],
            H=N.array(bondsys.loggedReconstructed.B[0].vals)[:,0,]),
                      H_plane=Struct(
            E=N.array(bondsys.loggedReconstructed.E[1].vals)[:,0,],
            H=N.array(bondsys.loggedReconstructed.B[1].vals)[:,0,]))
...
pickle.dump(Struct(res=res, drv_ts=drv_ts, E_plane_recon_pts=E_plane_recon_pts,
                   H_plane_recon_pts=H_plane_recon_pts),
            file('+pml_brick_cube_scat_16_1st_pml10_dt75.pickle', 'w'))
